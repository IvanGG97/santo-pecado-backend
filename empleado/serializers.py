...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        if user is None:
            raise AuthenticationFailed('Credenciales inválidas o la cuenta no ha sido activada.', 'no_active_account')

        if hasattr(user, 'empleado') and user.empleado.lockout_until and user.empleado.lockout_until > timezone.now():
            raise AuthenticationFailed('La cuenta ha sido bloqueada temporalmente por demasiados intentos fallidos.', 'account_locked')
        ...
        try:
            ...
            if hasattr(user, 'empleado'):
                ...
                if user.empleado.failed_login_attempts >= 3:
                    ...
                    user.empleado.save()
                    raise AuthenticationFailed('La cuenta ha sido bloqueada temporalmente.', 'account_locked')
                else:
...
class EmpleadoUpdateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if new_rol:
            ...
            if new_rol.name == 'Admin': user.is_staff = True
